...
extension PXDiscountDetailViewController {
    ...
    func getHeader() -> NSAttributedString {
        ...
        if amountHelper.consumedDiscount {
            return NSAttributedString(string: PXStrings.modal_title_consumed_discount.PXLocalized, attributes: attributes)
        } else {
    ...
    func getDisclaimer() -> NSAttributedString? {
        let attributes = [NSAttributedStringKey.font: Utils.getLightFont(size: PXLayout.XXS_FONT), NSAttributedStringKey.foregroundColor: ThemeManager.shared.greyColor()]
         if amountHelper.campaign?.maxRedeemPerUser == 1 {
            var message = PXStrings.unique_discount_detail_modal_footer.PXLocalized
            if let expirationDate = amountHelper.campaign?.endDate {
                let messageDate = PXStrings.discount_end_date.PXLocalized
            ...
            return NSAttributedString(string: PXStrings.multiple_discount_detail_modal_footer.PXLocalized, attributes: attributes)
         } else if amountHelper.consumedDiscount {
            return NSAttributedString(string: PXStrings.modal_content_consumed_discount.PXLocalized, attributes: attributes)
        }
        return nil
    }
