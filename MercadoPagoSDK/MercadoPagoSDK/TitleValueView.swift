...
class TitleValueView: UIView, PXComponent {
    ...
    public init(frame: CGRect, titleText: String, valueDouble: Double, colorTitle: UIColor =  UIColor.px_grayDark(), colorValue: UIColor =  UIColor.px_grayDark(), upperSeparatorLine: Bool = true, valueEnable: Bool = true, titleFontSize:CGFloat = FONT_SIZE, valueFontSize: CGFloat = FONT_SIZE) {
        super.init(frame: frame)
        let titleLabel = UILabel(frame:getTitleFrame())
        ...
        if valueEnable {
            let valueLabel = UILabel(frame: getValueFrame())
