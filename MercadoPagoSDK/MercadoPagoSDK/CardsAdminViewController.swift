...
open class CardsAdminViewController: MercadoPagoUIScrollViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func deleteCardAlertView(card: Card, message: String) {
        ...
        alert.addAction(UIAlertAction(title: "No".localized_temp, style: UIAlertActionStyle.cancel, handler: nil))
        alert.addAction(UIAlertAction(title: "Si".localized_temp, style: UIAlertActionStyle.default, handler: { (_) -> Void in
            self.callback(card)
        }))
