...
//
    ...
    public func updateCheckoutModel(paymentMethodSearch: PaymentMethodSearch) {
        ...
        if search?.getPaymentOptionsCount() == 0 {
            self.errorInputs(error: MPSDKError(message: "Ha ocurrido un error".localized, errorDetail: "No se ha podido obtener los métodos de pago con esta preferencia".localized, retry: false), errorCallback: { (_) in
