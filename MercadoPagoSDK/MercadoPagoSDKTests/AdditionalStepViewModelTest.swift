...
class ConfirmInstallmentsStepViewModelTest: BaseTest {
    var instance: ConfirmInstallmentsAdditionalStepViewModel!
    ...
    override func setUp() {
        ...
        let cardToken = MockBuilder.buildCardToken()
        let paymentMethod = MockBuilder.buildPaymentMethod("visa")
        self.instance = ConfirmInstallmentsAdditionalStepViewModel(amount: 20.0, token: cardToken, paymentMethod: paymentMethod, dataSource: payerCosts![0], discount: nil, email: nil)
    }
    ...
    func testScreenName() {
        XCTAssertEqual(self.instance!.getScreenName(), "CONFIRM_INSTALLMENTS")
    }
    ...
    func testRowsWithDiscount() {
        ...
        // Payer Costs Confirm cell
        XCTAssertTrue(self.instance!.showPayerCostDescription())
        XCTAssertTrue(self.instance!.isBodyCellFor(indexPath: IndexPath(row: 0, section: 3)))
        XCTAssertEqual(self.instance!.heightForRowAt(indexPath: IndexPath(row: 0, section: 3)), 260)
    }
